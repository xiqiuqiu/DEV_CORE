...
const DataPanel = ({ onThemeChange }: DataPanelProps) => {
  ...
  const uptimeHours = Math.max(
    0,
    Math.floor((now - launchDate) / (1000 * 60 * 60)),
  );
  ...
  const telemetryData = [
    ...
    {
      label: t.dataPanel.visitors,
      value: visitLoading ? "..." : (visitCount?.toLocaleString() ?? "0"),
    },
  ];
  ...
  return (
    <aside className="hidden lg:block fixed right-0 top-14 bottom-0 w-72 xl:w-80 border-l border-border bg-card text-card-foreground overflow-y-auto z-30">
      ...
      <div className="p-4 border-b border-border">
        ...
        <div className="relative w-full overflow-hidden border border-border bg-background">
          ...
          <div className="relative">
            <iframe
              src="//music.163.com/outchain/player?type=2&id=5256051&auto=0&height=66"
              width="100%"
              height="96"
              frameBorder="0"
              loading="lazy"
      ...
      <div className="p-4">
        ...
        <div
          className="bg-background p-3 border border-border"
